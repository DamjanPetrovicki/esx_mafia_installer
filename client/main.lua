...
function OpenCloakroomMenu()
	...
	if grade == 'recruta' then
		table.insert(elements, {label = _U('mafia_wear'), value = 'recruta_wear'})
	elseif grade == 'soldado' then
		table.insert(elements, {label = _U('mafia_wear'), value = 'soldado_wear'})	
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		if PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'gerente' then
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_mafiajob:getVehicleFromPlate', function(owner, found)
				if found then
...
AddEventHandler('esx_mafiajob:hasEnteredMarker', function(station, part, partNum)
	...
	elseif part == 'HelicopterSpawner' then
		...
		if not IsAnyVehicleNearPoint(helicopters[partNum].SpawnPoint.x, helicopters[partNum].SpawnPoint.y, helicopters[partNum].SpawnPoint.z,  3.0) then
			ESX.Game.SpawnVehicle('as350', helicopters[partNum].SpawnPoint, helicopters[partNum].Heading, function(vehicle)
...
function CreateBlip()
	if PlayerData.job ~= nil and PlayerData.job.name == 'mafia' then   
        ...
        blip = AddBlipForCoord(Config.Blip.Blip.Pos.x, Config.Blip.Blip.Pos.y, Config.Blip.Blip.Pos.z)
        SetBlipSprite(blip, Config.Blip.Blip.Sprite)
		SetBlipDisplay(blip, Config.Blip.Blip.Display)
		SetBlipScale  (blip, Config.Blip.Blip.Scale)
        SetBlipColour(blip, Config.Blip.Blip.Colour)
        SetBlipAsShortRange(blip, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(_U('map_blip'))
        EndTextCommandSetBlipName(blip)
